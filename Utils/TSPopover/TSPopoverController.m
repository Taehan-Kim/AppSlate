...
{
    // 중복 팝업 체크.
    if( USERCONTEXT.showPopover )
        return nil;

    USERCONTEXT.showPopover = YES;
...
//    self.contentView.frame = CGRectMake(senderPoint.x-10, senderPoint.y, 20, 30);
    ...
    popoverView.alpha = 0;
//    popoverView.frame = CGRectMake(senderPoint.x-10, senderPoint.y, 20, 30);
...
- (void) dismissPopoverAnimatd:(BOOL)animated
{
    if (self.view) {
        ...
        USERCONTEXT.showPopover = NO;  // blade.
    }
}
