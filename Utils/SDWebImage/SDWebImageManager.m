...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        asiDownloaders = [[NSMutableArray alloc] init];
    }
    return self;
}
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if (image)
    {
        ...
        if ([delegate respondsToSelector:@selector(webImageManager:didFinishFailWithImage:)]) {
            [delegate performSelector:@selector(webImageManager:didFinishFailWithImage:) withObject:self withObject:nil];
        }
    }
}
