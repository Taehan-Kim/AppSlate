...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = [paths[0] stringByAppendingPathComponent:@"ProfileCache"];
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    NSDictionary *dicdata = [keyAndData count] > 1 ? keyAndData[1] : nil;
...
- (NSDictionary *)dataFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    NSDictionary *dicdata = memCache[key];
    if (!dicdata) {
